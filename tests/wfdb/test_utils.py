...
class TestIsRecord(object):
    ...
    def test_non_existent_record(self):
        ...
        assert not utils.is_record(
            self.resource_path / "no_such_record_123", dat_ext=None
        )
        assert not utils.is_record(
            self.resource_path / "no_such_record_123", dat_ext=None, ann_ext="foo"
        )

    def test_ann_only_record(self):
        assert utils.is_record(self.resource_path / "foo", ann_ext="bar")
        assert utils.is_record(self.resource_path / "foo", ann_ext="bar", dat_ext=None)
