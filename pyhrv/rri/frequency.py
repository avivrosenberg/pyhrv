...
def pxx_lomb(rri: np.ndarray, f_axis: np.ndarray, trr: np.ndarray = None,
             t_win: float = None, win_func: Callable = sps.windows.hamming):
    """
    ...
    for win_idx, win_start in enumerate(window_starts):
        ...
        pxx_win = sps.lombscargle(trr, rri, w_axis,
...
def build_uniform_freq_axis(t_win: float, f_min: float, f_max: float,
